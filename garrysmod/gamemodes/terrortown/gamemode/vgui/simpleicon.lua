...
function PANEL:PaintOverHovered()
   ...
   surface.SetDrawColor(midnight_ui.panel.top_dark)
...
function PANEL:PressedAnim(anim, delta, data)
   ...
   if anim.Finished then self.Icon:StretchToParent(0, 0, 0, 0) return end
...
function PANEL:Init()
   self:SetIconText("")
   self:SetIconTextColor(midnight_ui.text_colour.white)
   self:SetIconFont("midnight_font_13")
   self:SetIconTextShadow({opacity = 200, offset = 1})
